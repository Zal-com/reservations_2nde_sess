    ...
    <div class="max-w-sm bg-white border border-gray-200 rounded-lg shadow my-2 mx-auto">
        ...
        <div class="p-5 m-h-300">
            ...
            <table class="w-[100%] card-class" >
                <thead>
                <th>{{$item->price}} € </th>
                <th>{{$item->representationsString()}}</th>
                <th>{{$item->bookableString()}}</th>
                </thead>
            </table>
            <div class="flex justify-end">
                <a
                    href="{{route('show.show', ['slug' => $item->slug])}}"
                    class="inline-flex items-center px-3 py-2 mt-4 text-sm font-medium text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4
